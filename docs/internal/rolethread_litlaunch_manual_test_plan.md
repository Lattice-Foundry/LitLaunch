...
| ID | Status | Effort | Command / Action | Expected Result | Record If It Fails | Severity | Feature Area |
| --- | --- | --- | --- | --- | --- | --- | --- |
| DIAG-01 | Not run | Light | `litlaunch inspect --profile rolethread-webapp` | Human guidance says diagnostics artifacts are available; no legacy full text report. | Output text. | Medium | Inspect UX |
| DIAG-02 | Not run | Light | `litlaunch inspect --profile rolethread-webapp --json > .\inspect-rolethread.json` | JSON includes runtime governance, exposure, transport security, profile/target info, and no obvious secrets. | JSON file excerpt and missing keys. | High | JSON diagnostics |
| DIAG-03 | Not run | Light | `litlaunch report --profile rolethread-webapp --open --force` | HTML opens, sections are readable, long paths wrap, privacy warning is visible. | Screenshot and generated file path. | High | HTML diagnostics |
| DIAG-04 | Not run | Medium | `litlaunch inspect app.py --host 127.0.0.1 --trust-mode strict_local --json` | Reports loopback/local posture without alarming network-exposure language. | Runtime Governance / Runtime Exposure excerpt. | Medium | Trust mode |
| DIAG-05 | Not run | Medium | `litlaunch inspect app.py --host 0.0.0.0 --trust-mode strict_local --json` | Reports strict-local violation/error posture. | JSON severity and recommendation. | High | Exposure guardrail |
| DIAG-06 | Not run | Medium | `litlaunch inspect app.py --host 0.0.0.0 --trust-mode internal_network --json` | Reports missing acknowledgement or blocked posture. | JSON posture fields. | High | Governance |
| DIAG-07 | Not run | Medium | `litlaunch inspect app.py --host 0.0.0.0 --trust-mode internal_network --allow-network-exposure --json` | Reports acknowledged network-visible posture with warning. | Acknowledgement and severity fields. | High | Governance |
| DIAG-08 | Not run | Medium | `litlaunch report app.py --host 0.0.0.0 --trust-mode internal_network --allow-network-exposure --force` | HTML report clearly warns about network-visible exposure. | Screenshot of governance/exposure sections. | High | Report UX |
| DIAG-09 | Not run | Medium | `litlaunch inspect app.py --host 0.0.0.0 --trust-mode internal_network --allow-network-exposure --streamlit-flag server.sslCertFile=cert.pem --streamlit-flag server.sslKeyFile=key.pem --json` | Transport Security reports Streamlit-native TLS configured and does not imply authentication. | Transport section excerpt. | Medium | TLS posture |
| DIAG-10 | Not run | Medium | `litlaunch inspect app.py --host 0.0.0.0 --trust-mode internal_network --allow-network-exposure --streamlit-flag server.sslCertFile=cert.pem --json` | Transport Security reports incomplete TLS config as warning/error. | Severity and recommendation. | Medium | TLS posture |
| DIAG-11 | Not run | Medium | `litlaunch inspect --profile rolethread-webapp --bundle --output rolethread-support.txt --force` then inspect the file. | Bundle includes privacy disclaimer, redaction wording, governance posture, and no obvious secrets. | Bundle excerpt and sensitive-looking values. | High | Support bundle |
...
| CON-05 | Not run | Medium | `litlaunch app.py --host 0.0.0.0 --trust-mode internal_network --allow-network-exposure` then stop with `Ctrl+C`. | Runtime warning mentions network exposure and plaintext HTTP if TLS is absent. | Warning text. | High | Security messaging |
