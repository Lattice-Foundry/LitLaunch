...
Experimental host sizing is separate from monitoring. When an app explicitly
sets `host_sizing = "initial"` or `host_sizing = "continuous"`, the launcher
may use exact window authority for bounded height-only fitting. Monitoring
itself remains observational, and host sizing never owns, kills, or closes
browser processes.
